package MigrateAcct::PageCreateAccount;
...
use MigrateAcct::Config   qw( MAKEUSER SUDO TITLE TOKEN_DIR COVERED DOMAIN );
...
sub run {
    ...
    $Info{ sponsor } =~ s/@{[ COVERED ]}/@{[ DOMAIN ]}/;

    log_it( 'creating account for sponsor ' . $Info{ sponsor } . '; alternate ' . $Info{ alternate } , $Info{ account } );
	...
	log_it( 'Success', $Info{ account } );
        ...
        mail( $Info{ sponsor }, 'banjo.employees.org account creation SUCCESS',
              <<"EOF_SUCCESS" );
...
EOF_SUCCESS
    ...
    otherwise {
        my $e = shift;
        mail( $Info{ sponsor }, 'banjo.employees.org account creation FAILURE',
              <<"EOF_FAILURE" );
...
sub send_page {
    ...
    print( "Your account ($Info{ account }) is being created. You
            will be mailed with the results.\n" );
}
