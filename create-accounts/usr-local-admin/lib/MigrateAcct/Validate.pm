...
use MigrateAcct::Config qw( DOMAIN LEGAL_DOMAINS PASS_MAX_LEN PASS_MIN_LEN LOG_FILE
               TOKEN_DIR TOKEN_LEN );
...
my %GOTO_MAP = ( GetSponsor => { load => 'MigrateAcct::PageGetSponsor',
                                 run  => \&MigrateAcct::PageGetSponsor::run },
                 GetInfo    => { load => 'MigrateAcct::PageGetInfo',
                                 run  => \&MigrateAcct::PageGetInfo::run },
               );
...
sub validate_account {
    ...
    if( getpwnam( $account ) or getgrnam( $account ) or _check_aliases( $account )  )
    {
        throw Error( -text  => "ERROR: The account you requested ($account) " .
                               ...
                               "another account name.  If you need to recover ".
			       "the password of your banjo account, use the " .
			       "<a href=\"$url/resetpw.cgi\">password reset" .
			       "</a> tool.",
                     -value => 'GetSponsor' );
    }
...
sub validate_alternate {
    ...
    if( url_param( 'token' ) ) {
        _load_token_file();
        $alternate = $Token_info{ alternate };
    }
    else {
        $alternate = param( 'alternate' ) || '';
    }

    # Make sure it is a legal address.
	...
	if ( $alternate =~ /\@.*employees.org$/ ||
	     $alternate =~ /\@.*cisco.com$/) {
	     throw Error( -text => "Error: alternate email address must not be ".
	                           "a employees.org or cisco.com address",
                      -value => 'GetSponsor' );
	}
...
sub _load_token_file {
    ...
    chomp( $Token_info{ account          } = <TOKEN> )
                                          or die "Bad token file ($token_file)";
    chomp( $Token_info{ alternate        } = <TOKEN> )
                                          or die "Bad token file ($token_file)";
...
sub log_it {
    ...
    {
	print LOG "[$timestamp] MigrateAcct $type: account: $account IP: $remote_addr\n";
	close(LOG);
    }
}
